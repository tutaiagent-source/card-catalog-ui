-- Canonical start_direct_conversation
-- Fixes conflicts from multiple prior overrides (013 + 023), while keeping:
-- - card-context messaging requires a listing context (p_context_card_id)
-- - cardless messaging (p_context_card_id IS NULL) allowed only after friendship acceptance
-- - direct conversation reuse is scoped to (target, context_card_id)
...
begin
  ...
  -- Reuse only the matching conversation scope:
  -- - card-context thread: same target + same context_card_id
  -- - cardless thread: same target + context_card_id IS NULL
  select c.id
  ...
  if v_conversation_id is null then
    ...
    -- If we are starting cardless messaging, ensure this thread is allowed for cardless inserts.
    if p_context_card_id is null then
