-- Harden confirm_deal_payment_and_mark_sold against concurrency/race conditions.
-- Adds row locking (FOR UPDATE) and idempotency for already-confirmed deals.
...
begin
  ...
  -- Lock the deal record so concurrent confirms serialize per deal.
  select *
  into v_deal
  ...
  v_card_id := v_deal.card_id;

  -- If already confirmed, be idempotent and just ensure the card is sold.
  if lower(coalesce(v_deal.status, '')) = 'payment_confirmed' then
    update public.cards
    set status = 'Sold',
  ...
  -- Lock the card row so confirmations for the same listing serialize.
  ...
  -- 1) Update deal_records status (guard against rare concurrent status flips).
  update public.deal_records
  set status = 'payment_confirmed'
  where id = v_deal.id
    and lower(coalesce(status,'')) <> 'payment_confirmed';

  get diagnostics v_rowcount = row_count;
  if v_rowcount = 0 then
    -- Either already confirmed by another request, or deal state changed unexpectedly.
    -- Be idempotent if it was already confirmed.
    return;
  end if;
  ...
  -- 3) Cancel other pending offers and decline other deal records on the same card.
